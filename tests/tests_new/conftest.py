"""
Pytest configuration for tests_new aligned with current package structure.
"""
...
async def perpetual_ai_engine() -> AsyncGenerator[PerpetualAIThinkingEngine, None]:
    ...
    engine = PerpetualAIThinkingEngine(database_url="sqlite:///:memory:", ai_config=ai_config)
    yield engine
...
def sample_cycle_data() -> dict:
    return {
        ...
        "priority": 3,
        "ai_enhanced": False,
        "config": {"max_iterations": 3, "confidence_threshold": 0.8},
    }
...
def sample_solution_data() -> dict:
    return {
        ...
        "components": [
            {"name": "Component 1", "description": "First component", "priority": 1, "estimated_effort": "1 week"},
            {"name": "Component 2", "description": "Second component", "priority": 2, "estimated_effort": "2 weeks"},
        ],
    }
...
def mock_enterprise_result() -> dict:
    return {
        ...
        "processing_time": 2.5,
        "metadata": {"enterprise": "test_enterprise", "timestamp": datetime.now(timezone.utc).isoformat()},
    }
